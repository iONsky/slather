...
module Slather
  class Project < Xcodeproj::Project
    ...
    attr_accessor :build_directory, :ignore_list, :ci_service, :coverage_service, :coverage_access_token, :source_directory,
      ...
      :decimals, :llvm_version, :configuration, :path_equivalence
    ...
    def pathnames_per_binary(binary_path)
      ...
      coverage_json["data"].reduce([]) do |result, chunk|
        result.concat(chunk["files"].map do |file|
          ...
          if self.path_equivalence
            path_equivalences = self.path_equivalence.split(",")
            temp_path = path_equivalences[0]
            target_path = path_equivalences[1]
            filename = filename.sub(temp_path, target_path)
          end
    ...
    def unsafe_llvm_cov_export_output(binary_path)
      ...
      if self.path_equivalence
        llvm_cov_args << "--path-equivalence" << self.path_equivalence
      end
